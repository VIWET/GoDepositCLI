...
- `--start-index`
    ...
    The index of the key from which `staking-cli` will start deposits and keystores or BLS to Execution messages generating. For example, if you generated `N` keys using a specific mnemonic, then for the next batch of keys you must use the `--start-index=N` flag.
...
- `--number`
    ...
    The number of keys (for deposits and keystores or for bls-to-execution messages) that will be generated by `staking-cli` starting from `--start-index`.

- `--amounts`

	Aliases: `--amount`, `-a`

	The amount of Ehter to deposit. You can specify a particular amount for each key index using `--amount=<key_index>:<amount>` as value, or set the default amount for all deposits setting `--amount=<amount>`.
...
- `--mnemonic-bitlen`
    ...
    The strength of the seed phrase, you can set it to `128`, `160`, `192`, `224`, `256`. These values affect the length of a mnemonic phrase, so you will generate 12 words, 15 words, 18 words, 21 words or 24 words. You can change the strength of your mnemonic in interactive mode.
