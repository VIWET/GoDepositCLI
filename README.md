...
- `--start-index`
	...
	The index of the key from which `staking-cli` will start deposits and keystores generation. For example, if you generated `N` keys using specific mnemonic, than for the next batch of keys you must use `--start-index=N` flag.
...
- `--number`
	...
	The number of keys (deposits and keystores) that will be generated by `staking-cli` starting from `--start-index`.
...
- `--directory`
	...
	The directory where all generated deposits and keystores will be stored.
...
- `--chain`
	...
	The chain, which configurations will be used for signing domain calculation. Bahamut chain version support `mainnet`, `sahara` and `horizon` values, Ethereum version support `mainnet` and `holesky` values. If you set unknown network name, you must specify `--genesis-fork`  and `--genesis-validators-root`.
